...
const getDefaultValues = (user) => {
  ...
  return {
    jobRequestFor:
      profile.relationship && profile.relationship !== "_self" ? "other" : "me",
...
const CustomerJobProfileStep = ({ onComplete, onBack }) => {
  ...
  const apiPut = useApiPut("/users/profile", (resp) => {
    ...
    if (exitAfterSaveInputRef.value === 1) {
      ...
      onComplete({ ...formData, _profileUpdates: resp });
    }
  });
  ...
  return (
    <form onSubmit={onSubmit} ref={formRef}>
      ...
      >
        {apiPut.isLoading ? "..." : <span>SAVE &amp; EXIT</span>}
      </Button>
    </form>
  );
};

CustomerJobProfileStep.propTypes = {
  onComplete: PropTypes.func.isRequired,
  onBack: PropTypes.func.isRequired,
};

export default CustomerJobProfileStep;
